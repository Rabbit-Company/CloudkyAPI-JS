...
describe("file list", () => {
	test("invalid server", async () => {
		const res: FileListResponse = await CloudkyAPI.listFiles("invalid server", username, token);
		expect(res.error).toBe(Error.SERVER_UNREACHABLE);
	});

	test("invalid username", async () => {
		const res: FileListResponse = await CloudkyAPI.listFiles(server, "test.test123", token);
		expect(res.error).toBe(Error.INVALID_USERNAME_FORMAT);
	});

	test("invalid token", async () => {
		const res: FileListResponse = await CloudkyAPI.listFiles(server, username, "test");
